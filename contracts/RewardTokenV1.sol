...
contract RewardTokenV1 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    struct Fees {
        uint256 buy_marketing_fees;
        uint256 sell_marketing_fees;
        uint256 buy_liquidity_fee;
        uint256 sell_liquidity_fee;
        uint256 buy_rewards_fee;
        uint256 sell_rewards_fee;
    ...
    constructor(
        ...
        marketingWallet = payable(marketing_wallet);
        devWallet = payable(dev_wallet);
        address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

        buyDeadFees = 0;
        sellDeadFees = 0;
        buyMarketingFees = fees.buy_marketing_fees;
        sellMarketingFees = fees.sell_marketing_fees;
        buyLiquidityFee = fees.buy_liquidity_fee;
        sellLiquidityFee = fees.sell_liquidity_fee;
        buyRewardsFee = fees.buy_rewards_fee;
        sellRewardsFee = fees.sell_rewards_fee;
        buyDevFee = 0;
        sellDevFee = 0;
        ...
        dividendTracker = new DividendTracker(
            ...
            router,
            token,
            string(abi.encodePacked(_name, "TRACKER")),
            string(abi.encodePacked(_symbol, "TRACKER"))
        );
        ...
        _mint(owner(), _supply); // only time internal mint function is ever called is to create supply
        maxWallet = _supply / 200000; // 0.005%
        swapTokensAtAmount = _supply / 2000000; // 0.05%;
...
contract DividendTracker is DividendPayingToken, Ownable {
    ...
    RewardTokenV1 public rewardToken;
    ...
    constructor(
        ...
        defaultToken = token;
        rewardToken = RewardTokenV1(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Fish_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Fish_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Fish contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            {
                uint256 received = rewardToken
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    rewardToken.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
